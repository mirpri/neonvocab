...
const LearningSession: React.FC<LearningSessionProps> = ({
  ...
  useEffect(() => {
    ...
    const definitionLen = definitionData.definition?.length ?? 0;
    const exampleLen = definitionData.exampleSentence?.length ?? 0;
    const shouldBeWide = definitionLen + exampleLen > 150;
    setContainerWidthClass(shouldBeWide ? "max-w-3xl" : "max-w-2xl");
  }, [definitionData]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Treat empty submit as requesting a hint (no error shake)
  ...
  let cardStyle =
    "bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-2xl border transition-all duration-300 relative overflow-hidden ";
  if (state === GameState.SUCCESS) {
    cardStyle += showNoIncrementNote
      ? "border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.3)]"
      : "border-green-500 shadow-[0_0_60px_rgba(34,197,94,0.4)]";
  } else if (state === GameState.SHOWING_ANSWER) {
    cardStyle += "border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.35)]";
  } else {
      ...
      <div className={cardStyle}>
        ...
        <form onSubmit={handleSubmit} className="relative">
          <input
            ...
            className={`w-full bg-slate-50 dark:bg-slate-900 border-2 rounded-2xl p-5 text-center text-2xl font-bold tracking-wide outline-none transition-all
              ...
              ${
                ...
                ${
                  state === GameState.WAITING_FOR_INPUT && !isShaking
                    ? "border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:border-indigo-500 focus:shadow-[0_0_20px_rgba(99,102,241,0.3)]"
                    : ""
                }
                ${
                  isShaking
          ...
          {state === GameState.WAITING_FOR_INPUT && input.length > 0 && (
            <button
              type="submit"
              className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-colors text-white"
            >
        ...
        {hintLevel > 0 && state === GameState.WAITING_FOR_INPUT && (
          <div className="mt-4 text-center animate-pop">
            <p className="text-indigo-600 dark:text-indigo-300 font-mono text-lg tracking-widest">
              {getHintText()}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {mistakes > 0
                ? "Be careful, mistakes reset progress!"
                : "Hint active"}
            </p>
          </div>
        )}
